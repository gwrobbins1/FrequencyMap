...
std::vector <MeasuredPoint> floodFill(MeasuredPoint current, double interval, std::vector<MeasuredPoint> sensorLoc, std::vector <MeasuredPoint> estimatedPoints)
{
	...
	for (n = 1; n < sensorLoc.size(); n++)
	{
		...
		if (sensorLoc.at(n).x < xMin)
			xMin = sensorLoc.at(n).x;
		if (sensorLoc.at(n).y > yMax)
			yMax = sensorLoc.at(n).y;
...
int main(int argc, char* argv[])
{
	...
	while( i < argc) 
	{
		temp.x = std::stod(argv[i]);
		temp.y = std::stod(argv[i+1]);
		temp.measuredStrength = std::stod(argv[i+2]);
